...
NUM_STEPS = 1000
LAMBDA_VALS = [0, 1e-5, 1e-4, 1e-3]
RESIDUAL_SCALES = [0.03, 0.1, 0.3]
HIDDEN_DIM = 32
FOLDS = 5


LEARNED_STRATEGY = "mlp_log_singular_values"
...
def main():
        ...
        for lambda_value in LAMBDA_VALS:
            for residual_scale in RESIDUAL_SCALES:
                for fold_idx in range(FOLDS):
                    ...
                    print(
                        f"rank={rank} lambda={lambda_value:g} "
                        f"residual_scale={residual_scale:g} fold={fold_idx} "
    ...
    summary_df = raw_df.groupby(["rank", "lambda", "residual_scale"], as_index=False).agg(
        mean_val_relative_mse=("val_relative_mse", "mean"),
        std_val_relative_mse=("val_relative_mse", "std"),
        mean_condition=("condition", "mean"),
        mean_rel_g_change_from_svd_sqrt=("rel_g_change_from_svd_sqrt", "mean"),
    )
    ...
    raw_df.to_csv(out_dir / "kfold_mlp_raw.csv", index=False)
    summary_df.to_csv(out_dir / "kfold_mlp_summary.csv", index=False)
    best_df.to_csv(out_dir / "kfold_mlp_selected_hparams.csv", index=False)
