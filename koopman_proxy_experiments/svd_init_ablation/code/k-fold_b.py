...
def main():
    ...
    for rank in RANKS:
        ...
        for lambda_value in LAMBDA_VALS:
            for fold_idx in range(FOLDS):
                ...
                optimizer = torch.optim.Adam([b], lr=1e-2)
    ...
    raw_df.to_csv(out_dir / "kfold_b_lambda_raw.csv", index=False)
    summary_df.to_csv(out_dir / "kfold_b_lambda_summary.csv", index=False)
    best_df.to_csv(out_dir / "kfold_b_selected_lambdas.csv", index=False)
